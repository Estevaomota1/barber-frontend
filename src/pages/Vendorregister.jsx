...
export default function VendorRegister() {
  ...
  async function handleSubmit(e) {
    ...
    setError('')
    try {
      const res = await api.post('/register/vendor', form)
    ...
    <div style={s.page}>
      <div style={s.wrap}>
        <div style={s.logoWrap}>
          ...
          <p style={s.logoSub}>Acesso de Vendedor</p>
        </div>

        <div style={s.card}>
          <div style={s.vendorBadge}>
            <i className="ti ti-user-check" style={{ marginRight: '8px', color: '#818cf8' }}></i>
            Conta Vendedor
          </div>

          <h2 style={s.cardTitle}>Criar acesso de vendedor</h2>
          <p style={s.cardSub}>Com este acesso você pode cadastrar clientes no sistema.</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <button type="submit" disabled={loading} style={{ ...s.btn, marginTop: '4px' }}>
              {loading ? 'Criando acesso...' : 'Criar acesso de vendedor'}
            </button>
          </form>
...
const s = {
  ...
  vendorBadge: { display: 'inline-flex', alignItems: 'center', background: 'rgba(129,140,248,0.1)', border: '1px solid rgba(129,140,248,0.2)', color: '#818cf8', fontSize: '12px', fontWeight: '600', padding: '4px 12px', borderRadius: '20px', marginBottom: '14px' },
  ...
  btn: { width: '100%', background: '#818cf8', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '14px', fontWeight: '600', color: '#fff', cursor: 'pointer' },
}